-- ============================================================
-- MIGRATION 021 — Qualify pgcrypto functions for production.
--
-- Production installs pgcrypto in the extensions schema while this
-- security-definer function uses search_path = public. Qualify every
-- pgcrypto call so activation-code generation resolves reliably.
--
-- Applying this migration only replaces the function definition. It does
-- not execute the function, generate a code, create a claim, revoke a
-- claim, or modify any existing business row.
-- ============================================================
...
begin
  ...
  -- Revoke only the previous pending claim for this business and email.
  ...
  v_code := 'LUM-' || upper(translate(encode(extensions.gen_random_bytes(12), 'base64'), '+/=', ''));
...
select '021_fix_owner_activation_crypto_schema applied' as status;
